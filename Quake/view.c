...
==================
*/
void V_ChaseCam (void)
{
	if (chase_active.value) {
		Cvar_SetValue("chase_active", 0); // toggle off
		SCR_CenterPrint ("chase camera: off");
		return;
	}

	if (!chase_active.value) {
		Cvar_SetValue("chase_active", 1); // toggle on
		SCR_CenterPrint ("chase camera: on");
		return;
	}
}
...
void V_Init (void)
{
	...
	Cmd_AddCommand ("chasecam", V_ChaseCam);
