...
// rotate binary objects locally
		...
		if (ent->effects & EF_MUZZLEFLASH)
		{
			...
			// jimhaworth -- use yellow/orange color for muzzle flashes
			dl->color[0] = 0.98f;
			dl->color[1] = 0.64f;
			dl->color[2] = 0.01f;
		...
		if (ent->effects & EF_BRIGHTLIGHT)
		{
			...
			// jimhaworth -- use yellow/orange color for bright lights
			dl->color[0] = 0.99f;
			dl->color[1] = 0.86f;
			dl->color[2] = 0.37f;
		}
		...
		else if (ent->model->flags & EF_ROCKET)
		{
			...
			// jimhaworth -- use yellow/orange color for rocket trails
			dl->color[0] = 0.98f;
			dl->color[1] = 0.64f;
			dl->color[2] = 0.01f;
		}
		else if (ent->model->flags & EF_GRENADE)
		{
			...
			// jimhaworth -- make the grenades glow red
			dl = CL_AllocDlight (i);
			VectorCopy (ent->origin, dl->origin);
			dl->radius = 100;
			dl->die = cl.time + 0.01;
...
//johnfitz -- devstats
	...
	for (i=0, b=cl_beams ; i< MAX_BEAMS ; i++, b++)
		if (b->model && b->endtime >= cl.time)
			num_beams++;
	if (num_beams > 64 && dev_peakstats.beams <= 64)
		Con_DWarning ("%i beams exceeded standard limit of 64 (max = %d).\n", num_beams, MAX_BEAMS);
