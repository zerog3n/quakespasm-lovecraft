...
static void PF_makestatic (void)
{
	...
	// jimhaworth -- yellow light for flames and walltorches
	string_t input_model  = SV_ModelIndex(PR_GetString(ent->v.model));	// 
	string_t flame_model  = SV_GetModelIndex("progs/flame.mdl"); 		// #89
	string_t flame2_model = SV_GetModelIndex("progs/flame2.mdl"); 		// #89

	if (input_model == flame_model || input_model == flame2_model) {
		Con_Printf ("light: %d ? %d '%f %f %f')\n", input_model, flame_model, ent->v.origin[0], ent->v.origin[1], ent->v.origin[2]);
		CL_DelayedLightEmitter(ent->v.origin, 300, 0.98f, 0.64f, 0.01f);
	}
