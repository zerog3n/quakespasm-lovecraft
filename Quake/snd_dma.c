...
=================
*/
int SND_MissileChannel(int channel)
{
	if (channel == CHAN_MISSILE_1) return true;
	if (channel == CHAN_MISSILE_2) return true;
	if (channel == CHAN_MISSILE_3) return true;
	if (channel == CHAN_MISSILE_4) return true;
	if (channel == CHAN_MISSILE_5) return true;
	if (channel == CHAN_MISSILE_6) return true;
	return false;
}
...
=================
*/
int SND_ImpactChannel(int channel)
{
	if (channel == CHAN_IMPACT_1) return true;
	if (channel == CHAN_IMPACT_2) return true;
	if (channel == CHAN_IMPACT_3) return true;
	if (channel == CHAN_IMPACT_4) return true;
	if (channel == CHAN_IMPACT_5) return true;
	if (channel == CHAN_IMPACT_6) return true;
	return false;
}
...
void SND_Spatialize (channel_t *ch)
{
	...
	dist = VectorNormalize(source_vec) * ch->dist_mult * 0.642;
	...
	if (shm->channels == 1) {
		...
		rscale = 1.0 + dot * 0.001;
		lscale = 1.0 - dot * 0.001;
	}

	// max distance for spatial sounds
	max_dist = 3;

	// add in distance effect
	// - adjust right sound volume
	scale = (1.0 - dist) * rscale;
	ch->rightvol = (int) (ch->master_vol * scale);
	if (ch->rightvol <= 0) {
		// set min volume if max distance not exceeded
		if (dist < max_dist) ch->rightvol = 2 * rscale;
		else ch->rightvol = 0;

		// don't completely silence missile or impact entities
		if (ch->is_missile || ch->is_impact) ch->rightvol = 10;
	}
	// - adjust left sound volume
	scale = (1.0 - dist) * lscale;
	ch->leftvol = (int) (ch->master_vol * scale);
	if (ch->leftvol <= 0) {
		// set min volume if max distance not exceeded
		if (dist < max_dist) ch->leftvol = 2 * lscale;
		else ch->leftvol = 0;

		// don't completely silence missile or impact entities
		if (ch->is_missile || ch->is_impact) ch->leftvol = 10;

		//Con_Printf ("entity %i at dist %f\n", ch->entnum, dist); //(debugging)
	}

	// show which channel is being used for missile and impact entities
	if (ch->is_missile) {
		//Con_Printf ("missile entity %i on channel %i\n", ch->entnum, ch->entchannel);
	}
	if (ch->is_impact) {
		//Con_Printf ("impact entity %i on channel %i\n", ch->entnum, ch->entchannel);
	}
}
...
// determine if the sound is for missiles or impacts
	is_missile = SND_MissileChannel(entchannel);
	is_impact  = SND_ImpactChannel(entchannel);
...
// spatialize
	...
	// missile or impact detection
	if (is_missile) target_chan->is_missile = true;
	if (is_impact) target_chan->is_impact = true;
