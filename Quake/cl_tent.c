...
void CL_ParseTEnt (void)
{
	...
	case TE_WIZSPIKE:			// spike hitting wall
		...
		S_StartSound (-1, CHAN_IMPACT_1, cl_sfx_wizhit, pos, 1, 1);
		break;
	...
	case TE_KNIGHTSPIKE:			// spike hitting wall
		...
		S_StartSound (-1, CHAN_IMPACT_1, cl_sfx_knighthit, pos, 1, 1);
		break;
	...
	case TE_SPIKE:			// spike hitting wall
		...
		if ( rand() % 5 )
			S_StartSound (-1, CHAN_IMPACT_1, cl_sfx_tink1, pos, 1, 1);
		else
		{
			rnd = rand() & 3;
			if (rnd == 1)
				S_StartSound (-1, CHAN_IMPACT_1, cl_sfx_ric1, pos, 1, 1);
			else if (rnd == 2)
				S_StartSound (-1, CHAN_IMPACT_1, cl_sfx_ric2, pos, 1, 1);
			else
				S_StartSound (-1, CHAN_IMPACT_1, cl_sfx_ric3, pos, 1, 1);
		}
		break;
	...
	case TE_EXPLOSION:			// rocket explosion
		...
		// jimhaworth -- use yellow/orange color for explosions
		dl = CL_LightEmitter(pos, 300, 0.98f, 0.64f, 0.01f, 0.5);

		S_StartSound (-1, CHAN_IMPACT_1, cl_sfx_r_exp3, pos, 1, 1);
		break;
	...
	case TE_TAREXPLOSION:	// tarbaby explosion
		...
		pos[2] = MSG_ReadCoord (cl.protocolflags);
		R_BlobExplosion (pos);
		// jimhaworth -- use purple color for explosions
		dl = CL_LightEmitter(pos, 300, 0.70f, 0.22f, 0.86f, 0.5);
		S_StartSound (-1, CHAN_IMPACT_1, cl_sfx_r_exp3, pos, 1, 1);
		break;
	...
	case TE_LIGHTNING2:		// lightning bolts -- player weapon
		CL_ParseBeam (Mod_ForName("progs/bolt2.mdl", true));
		break;

	case TE_LIGHTNING3:		// lightning bolts -- chthon 
...
// PGM 01/21/97
	...
	case TE_TELEPORT:
		...
		dl = CL_LightEmitter(pos, 250, 0.0f, 0.3f, 1.0f, 0.25);
		break;
